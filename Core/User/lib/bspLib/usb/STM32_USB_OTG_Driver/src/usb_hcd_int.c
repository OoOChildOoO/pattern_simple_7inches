...
//    if (gintsts.b.sofintr)				// start of frame					有接入U盘每一ms都进
//    {
//      retval |= USB_OTG_USBH_handle_sof_ISR (pdev);
//	  if(usbredyf){
//		  if(timedelay){
//			  timedelay --;
//			  if(timedelay==0)SETUSBLED1();
//		  }
//	  }
//    }
    
    if (gintsts.b.rxstsqlvl)			// RxFIFO non_empty					读写时进
    {
      retval |= USB_OTG_USBH_handle_rx_qlvl_ISR (pdev);
	  if(WUSBLED1)CLRUSBLED1();else SETUSBLED1();
	  timedelay = 10;
    }
    
    if (gintsts.b.nptxfempty)			// non_periodic TxFIFO empty		不进
    {
      retval |= USB_OTG_USBH_handle_nptxfempty_ISR (pdev);
    }
    
    if (gintsts.b.ptxfempty)			// periodic TxFIFO empty			不进
    {
      retval |= USB_OTG_USBH_handle_ptxfempty_ISR (pdev);
    }    
    
    if (gintsts.b.hcintr)				// host channels interrupt			读写时进	 一次
    {
      retval |= USB_OTG_USBH_handle_hc_ISR (pdev);
    }
    
    if (gintsts.b.portintr)				// host port interrupt				插U盘和拔U盘时进
    {
      retval |= USB_OTG_USBH_handle_port_ISR (pdev);
    }
    
    if (gintsts.b.disconnect)			// disconnect detected interrupt	拔U盘时进
    {
      retval |= USB_OTG_USBH_handle_Disconnect_ISR (pdev);  
      usbredyf = 0;
    }
    
    if (gintsts.b.incomplisoout)		// incomplete periodic transfer		不进
    {
