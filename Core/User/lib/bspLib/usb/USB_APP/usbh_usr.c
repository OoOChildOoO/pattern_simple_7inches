...
extern void USB_USR_MSC_FATFSRESET(void);
static u8 AppState; 
extern USB_OTG_CORE_HANDLE          USB_OTG_Core;

//USB OTG 中断服务函数
//处理所有USB中断			usb_hcd_int
...
u8 writedat[50] ={"MLK-Udisk Version-1.00 User-8 SN-15102010305060400"};
u8 IS_checkLogin(u8 *in,u8 *des,u16 len)
{
	u16 i;
	for(i=0;i<len;i++){
		if(i==28){	//记录登录用户等级
			if((*(in+i)>0x39) ||(*(in+i)<0x31)){	//用户登进仅支持1-9
				return 0;
			}
		}else{
			if(*(in+i) != *(des+i))	return 0;
		}
	}
...
int USBH_USR_MSC_Application(void)
{
	...
	u16 i;
	u8 f,buff[512] ;
	//printf("USBH_USR_MSC_Application AppState = %d\r\n",AppState);
  	switch(AppState)
  	{
...
//			printf("开始执行用户程序!!!\r\n");
			...
			f = USBH_UDISK_Read(buff,2,1); 
			f = IS_checkLogin(buff,writedat,50);
			//printf("f = %d writedat[28] = %c \r\n",f,writedat[28]);
			if(f==1){
				g_user.newuser = writedat[28] -0x30;
				g_user.olduser = g_user.newuser;
				g_user.newmode = 0;
				g_user.oldmode = 0;	
				g_usblogin = 0xa500|g_user.newuser;				
			}else{
				g_usblogin = 0;
			}
